...
class Rest_Helper {
    ...
    public static function upload_image_from_url( $image_url ) {
        ...
        if ( ! $parsed_url || ! is_array( $parsed_url ) ) {
            ...
            return new WP_Error( 'wpwax_customer_support_app_rest_invalid_image_url', sprintf( __( 'Invalid URL %s.', 'directorist' ), $image_url ), array( 'status' => 400 ) );
        }
        ...
        if ( isset( $file['error'] ) ) {
            ...
            /* translators: %s: error message */
            return new WP_Error( 'wpwax_customer_support_app_rest_invalid_image', sprintf( __( 'Invalid image: %s', 'directorist' ), $file['error'] ), array( 'status' => 400 ) );
        }
