...
function Sidebar() {
    ...
    const handleSessionSearch = (event) => {
        ...
        const generatedSessions = sessionList.filter((entry) =>
            entry.users.every((searchableEntry) =>
                searchableEntry.name.includes(keyword)
            )
        );
        setSessionState({
            ...sessionState,
    ...
    return (
        <SidebarWrap className={loader ? 'wpwax-vm-loder-active' : null}>
            <div className='wpwax-vm-sidebar-top'>
                ...
                <a href='#' className='wpwax-vm-sidebar-refresher'>
